//
// RobotActuator.cs - a hand-written Renode peripheral for lab 13.
//
// Models the robot's memory-mapped "motor controller" register block. The
// transformer policy on the host writes the currently-commanded skill here;
// this model mirrors the values and logs each skill to the Renode monitor
// so you can watch the hardware side of the plan execute.
//
// Register map (offsets from 0x90000000):
//   0x00 SKILL_ID   current skill (1=WALK 2=TURN 3=STOP 4=DOCK)
//   0x04 PARAM0     velocity (cm/s) or turn angle (deg)
//   0x08 PARAM1     distance (cm)
...
namespace Antmicro.Renode.Peripherals.Robot
{
    public class RobotActuator : IDoubleWordPeripheral, IKnownSize
    {
        ...
        public void WriteDoubleWord(long offset, uint value)
        {
            switch (offset)
            {
            case (long)Registers.SkillId: skillId = value; break;
            case (long)Registers.Param0:  param0 = value; break;
            case (long)Registers.Param1:  param1 = value; break;
            case (long)Registers.Step:    step = value; LogSkill(); break;
            default:
        ...
        private void LogSkill()
        {
            ...
            case 2: // TURN
                this.Log(LogLevel.Info,
                    "actuator step {0}: {1} angle={2} deg", step, name, param0);
                break;
            default:
                this.Log(LogLevel.Info, "actuator step {0}: {1}", step, name);
                break;
            }
        }
