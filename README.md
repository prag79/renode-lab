...
| `lab 08` | Multi-node IoT network: three FE310 machines on a shared UART hub, sensors reporting to a gateway | [`labs/08-multi-node-iot/`](labs/08-multi-node-iot/) |
| `lab 09` | Edge AI / TinyML: an int8-quantized neural net classifies handwritten digits with integer-only inference on a bare-metal RV64 core | [`labs/09-edge-ai/`](labs/09-edge-ai/) |
| `lab 10` | The real deal: unmodified **TensorFlow Lite Micro** (+ Zephyr) doing gesture recognition on a LiteX/VexRiscv SoC, plus an optional Verilated **CFU** hardware accelerator | [`labs/10-tflite-micro/`](labs/10-tflite-micro/) |
